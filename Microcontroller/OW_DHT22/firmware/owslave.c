...
bank1 byte ow_interrupt; 

volatile bank1 byte OW_serial[8];

#define SetState(state) { ow_status=state; ow_error=0; ow_buffer=0; ow_interrupt=1; }

unsigned char OW_interrupted(void)
{
  if (!ow_interrupt)
    return 0;
  ow_interrupt = 0;
...
void interrupt ISR(void)
{
  ...
  { 
    ...
    if ( ow_buffer == 0x55 ) // match rom
    {
      ...
      SetState( i==8 ? CMD_FUNCTION : CMD_NOMATCH);
    } else
