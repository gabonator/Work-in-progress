...
int InetGSM::httpOpen(const char* server, int port)
{
  ...
  */
  ...
  while(n_of_at<3){
		//Serial.println("gerr:connect");
	  if(!connectTCP(server, port)){
		  //Serial.println("gerr:cannotconn");
	    	n_of_at++;
	  }
	  else{
		return true;
...
int InetGSM::httpGET(const char* server, int port, const char* path, char* result, int resultlength)
{
  ...
  */
  ...
  if (resultlength > 0) 
    return gsm.read(result, resultlength);
  return 1;
...
/*
  gsm.SimpleWriteln("AT+CIFSR"); // OK?
  gsm.SimpleWriteln("AT+CIPSHUT"); // SHUT OK
  gsm.SimpleWriteln("AT+CST=\"apn\",\"\",\"\""); // OK
  gsm.SimpleWriteln("AT+CICR"); // SHUT OK
  gsm.SimpleWriteln("AT+CIFSR"); // OK
*/
...
int InetGSM::connectTCP(const char* server, int port)
{
  ...
  switch(gsm.WaitResp(1000, 200, "OK")){
	case RX_TMOUT_ERR: 
		//Serial.println("gerr:tcpRX_TMOUT_ERR");
		return 0;
	break;
	case RX_FINISHED_STR_NOT_RECV: 
		//Serial.println("gerr:tcpRX_FINISHED_STR_NOT_RECV");
		return 0; 
	break;
  }
		//Serial.println("gerr:tcpok!");
  #ifdef DEBUG_ON
