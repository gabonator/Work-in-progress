...
public:
     ...
     */
    REGISTER(uint8_t *val, const uint8_t len, TValueUpdaterFunction updateValH, TValueSetterFunction setValH, 
		EAccess _access=Public, const SWDTYPE _type=SWDTYPE_OTHER, const int eepromAddr=-1, const uint16_t bank=DEFAULT_NVOLAT_SECTION) : 
      id(regIndex++), value(val), length(len), type(_type),
	  access(_access), eepromAddress(eepromAddr), eepromBank(bank),
	  updateValue(updateValH), setValue(setValH)
    {
      pPrev = pLast;
      pLast = this;
    };
