...
{
	BOOL bAny = FALSE;
	int keys[] = {VK_RIGHT, VK_LEFT, VK_UP, VK_DOWN, 'Y', 'N', 'K', 'H', 'T', 'A', 'S', 'R', VK_CONTROL, VK_DELETE, 0};
	for (int i=0; keys[i]; i++)
		if ( GetAsyncKeyState(keys[i]) )
		{
			_data[0x6BD+i] = 0x00; // Y, N
			bAny = TRUE;
		}
		else
			_data[0x6BD+i] = 0x80; // Y, N
	if ( GetAsyncKeyState(' ') )
		bAny = TRUE;
	return bAny;
}
...
void _int(BYTE i)
{
	...
	if ( i == 0x1A )
	{
		...
		DWORD dwTime = (WORD)((FLOAT)GetTickCount()*65543.0f/(1000.0f*60.0f*60.0f));
		if ( ah == 0x00 )
		{
...
int main(int argc, char* argv[])
{
	...
	fclose(f);
	/*
	f = fopen("C:\\Data\\Work\\alley\\ds", "wb");
	fwrite(p1010, 0x7130, 1, f);
	fclose(f);
	*/
	ds = 0;
	memset(_data.raw, 0, 0x10000);
	memcpy(_data.raw, p1010, sizeof(p1010));
	//qub_1AC9C();
