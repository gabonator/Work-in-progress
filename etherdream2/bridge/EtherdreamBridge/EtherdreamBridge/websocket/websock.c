...
#endif
	ws_thread_info *tinfo = NULL, *current = state->tlist;
	tinfo = (ws_thread_info *) lws_malloc(sizeof(ws_thread_info));
	memset(tinfo, 0, sizeof(ws_thread_info));
	tinfo->thread = thread;
	tinfo->type = type;
...
void libwebsock_cleanup_thread_list(evutil_socket_t sock, short what, void *arg) {
	...
	ws_thread_info *tinfo, *current = NULL;
...
void libwebsock_post_shutdown_cleanup(evutil_socket_t fd, short what, void *arg) {
	...
	ws_thread_info *info = state->tlist;
